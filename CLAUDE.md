...
**One known defect: the widget does not notice the charger.** See "Battery state never
arrives by broadcast" under Gotchas — this is the next thing to fix, and it is an
...
**These do not currently repaint a placed widget** — nothing wakes the app (see Gotchas), so
you are waiting on the 15-minute alarm. To see a change immediately, either reinstall
(`MY_PACKAGE_REPLACED` does get through) or use `PreviewActivity`, which reads battery state
on create and has its own slider.
...
`PowerEventReceiver` is declared for `ACTION_POWER_CONNECTED`, `ACTION_POWER_DISCONNECTED`,
`BATTERY_LOW` and `BATTERY_OKAY`. **Android drops all four before they reach it** — the
Android 8+ implicit-broadcast restriction, verified on API 36 via `dumpsys activity
broadcasts`:
...
`android.os.action.CHARGING` / `DISCHARGING` do not get through either. `BOOT_COMPLETED` and
`MY_PACKAGE_REPLACED` **do** — they are on the exemption list, and the receiver's reboot and
reinstall handling genuinely works.

Consequences to keep straight:

- `exported="false"` is **not** the cause. `MY_PACKAGE_REPLACED` reaches the same receiver
  with the same flag. Do not "fix" this by exporting the receivers.
- The 15-minute alarm in `UpdateScheduler` is the only live refresh path, so charging state
  lags by up to 15 minutes. It works because it is our own `PendingIntent` (same UID).
  Confirmed scheduled via `dumpsys alarm`.
- The framework-only fix is `JobScheduler`: `setRequiresCharging(true)` for plug-in,
  `onStopJob` for unplug, `setRequiresBatteryNotLow(true)` for the low threshold. Not yet
  implemented — it is a deliberate open decision.
- `README.md` documents the same defect under "Staying current". Keep the two in step when
  the code changes.
